...
original_shared_prefix_file = os.path.join(script_dir, "perf-configs", "shared_prefix_agentic-1m-1k.yaml")
isolated_shared_prefix_file = os.path.join(script_dir, "perf-configs", "shared_prefix_agentic_passthrough.yaml")
router_config_file = os.path.join(script_dir, "perf-configs", "router-configs", "random-passthrough.yaml")
...
def update_files(token_size):
    ...
    print(f"\n=======================================================")
    print(f"Updating configs for token_size={token_size} (isolated passthrough)")
    print(f"  system_prompt_len = {system_prompt_len}")
    print(f"  question_len      = {question_len}")
    print(f"=======================================================")
    
    # Read from original template if isolated copy doesn't exist yet
    source_file = isolated_shared_prefix_file if os.path.exists(isolated_shared_prefix_file) else original_shared_prefix_file
    ...
    with open(isolated_shared_prefix_file, "w") as f:
        f.write(sp_content)

def main():
    test_name = "random-passthrough-10qps"
    ...
    for idx, token_size in enumerate(token_sizes, 1):
        ...
        cmd = [
            ...
            "--test-name", test_name,
            "--perf-job", isolated_shared_prefix_file,
            "--sim-replicas", "10",
    ...
    # Clean up isolated copy at the end
    if os.path.exists(isolated_shared_prefix_file):
        os.remove(isolated_shared_prefix_file)
