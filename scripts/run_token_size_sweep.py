...
router_config_file = os.path.join(script_dir, "perf-configs", "router-configs", "optimized-baseline-1m-prefix.yaml")
...
def update_files(token_size):
    ...
    rc_content = re.sub(r'maxPrefixTokensToMatch:\s*\d+', f'maxPrefixTokensToMatch: {token_size}', rc_content)
    
    with open(router_config_file, "w") as f:
        f.write(rc_content)

def main():
    test_name = "optimized-baseline-1m-prefix-10qps"
