...
The benchmark sweep (`scripts/run_token_size_sweep.py`) executed `scripts/run_nightly_perf.py` under the test name `optimized-baseline-1m-prefix-10qps`, systematically scaling `maxPrefixTokensToMatch`, `system_prompt_len`, and `question_len` for each step.
...
- **EPP CPU Usage scales from ~0.9 cores to ~7.1 cores**: Peak compute usage grows from **871m (~0.87 cores) at 1k tokens** to **7,120m (~7.12 cores) at 1M tokens** (+6,893m over idle baseline) due to the computational complexity of longest-prefix matching across 10 candidate endpoint pods.
...
- **EPP CPU Scaling:**
  ...
  - *Root Cause:* Evaluating longest prefix matches across 10 model-server candidates at 10 requests per second requires traversing deep tree branches for up to 1,000,000 token IDs per request.
